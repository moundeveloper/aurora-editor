...
import {
  ChevronDown, ChevronLeft, ChevronRight, Copy, Diamond, Eye,
  Link2, MoreHorizontal, Plus, RotateCcw, SlidersHorizontal, Sparkles,
  Clock3, Trash2,
} from '@lucide/vue'
...
function setValue(key: TransformKey, event: Event) { store.setLayerValue(key, Number((event.target as HTMLInputElement).value)) }
function hasKeyAtTime(key: TransformKey) {
  const tolerance = (0.5 / project.value.frameRate) + 0.0001
  return selectedLayer.value?.transform[key].keyframes.some((item) => item.id === selectedKeyframeId.value || Math.abs(item.time - currentTime.value) <= tolerance) ?? false
}
      ...
      <div class="inspector-scroll">
        <section class="property-section">
          ...
          <div v-if="!collapsed.transform" class="property-list">
            <div v-for="property in transformProperties" :key="property.key" class="property-row">
              ...
              <label :title="`Drag to scrub ${property.label}`">{{ property.label }}</label>
              <div class="numeric-field">
                <input :value="valueFor(property.key)" type="number" :min="property.min" :max="property.max" :step="property.step" @input="setValue(property.key, $event)" />
                <span v-if="property.suffix">{{ property.suffix }}</span>
              </div>
              <div class="key-controls">
                <button type="button" title="Previous keyframe"><ChevronLeft :size="10" /></button>
                <button type="button" title="Add or remove keyframe" :class="{ keyed: hasKeyAtTime(property.key), animated: selectedLayer.transform[property.key].animated }" @click="store.addKeyframe(property.key)"><Diamond :size="10" :fill="hasKeyAtTime(property.key) ? 'currentColor' : 'none'" /></button>
                <button type="button" title="Next keyframe"><ChevronRight :size="10" /></button>
              </div>
        ...
        <section class="property-section effects-section">
          ...
          <article v-for="effect in selectedEffects" :key="effect" class="effect-panel">
            ...
            <div v-for="parameter in effectDefinitions[effect] || []" :key="parameter.label" class="effect-parameter"><span>{{ parameter.label }}</span><div>{{ parameter.value }}</div><Diamond :size="9" /></div>
          </article>
...
.property-list { padding: 5px 4px 7px; }.property-row { position: relative; display: grid; min-height: 27px; grid-template-columns: 18px minmax(54px, .9fr) minmax(65px, 1fr) 50px; align-items: center; gap: 3px; padding: 1px 2px; }.property-row label { overflow: hidden; color: var(--text-secondary); font-size: 9.5px; text-overflow: ellipsis; white-space: nowrap; cursor: ew-resize; }.animate-toggle { display: grid; width: 18px; height: 18px; place-items: center; padding: 0; color: #565b66; background: transparent; border: 0; border-radius: 3px; cursor: pointer; }.animate-toggle:hover { color: var(--text-secondary); background: var(--bg-hover); }.animate-toggle.active { color: var(--keyframe); }.numeric-field { display: flex; height: 23px; min-width: 0; align-items: center; overflow: hidden; color: var(--text-muted); background: var(--bg-input); border: 1px solid var(--border-strong); border-radius: 3px; font-size: 9px; }.numeric-field:focus-within { border-color: var(--focus); }.numeric-field input { width: 100%; min-width: 0; height: 100%; padding: 0 5px; color: var(--text-primary); background: transparent; border: 0; outline: 0; font: inherit; font-size: 9.5px; font-variant-numeric: tabular-nums; }.numeric-field span { padding-right: 5px; }.numeric-field.time { justify-content: flex-end; padding: 0 5px; color: var(--text-secondary); font-variant-numeric: tabular-nums; }.key-controls { display: flex; }.key-controls button { display: grid; width: 16px; height: 19px; place-items: center; padding: 0; color: #50545e; background: transparent; border: 0; cursor: pointer; }.key-controls button:hover { color: var(--text-primary); }.key-controls button.animated { color: #93815f; }.key-controls button.keyed { color: var(--keyframe); }.link-button { position: absolute; z-index: 2; right: 52px; bottom: -13px; display: grid; width: 16px; height: 16px; place-items: center; padding: 0; color: var(--text-muted); background: var(--bg-panel); border: 0; cursor: pointer; }.link-button.active { color: var(--accent); }.row-indent { width: 18px; }.property-list.simple .property-row { grid-template-columns: 18px 1fr minmax(82px, 1.15fr); }.select-control { display: flex; height: 23px; align-items: center; justify-content: space-between; padding: 0 6px; color: var(--text-secondary); background: var(--bg-input); border: 1px solid var(--border-strong); border-radius: 3px; font: inherit; font-size: 9.5px; cursor: pointer; }
.effect-panel { margin: 5px 6px; background: #15171d; border: 1px solid #292d36; border-radius: 3px; }.effect-panel header { display: flex; height: 27px; align-items: center; gap: 5px; padding: 0 4px; border-bottom: 1px solid #282b33; }.effect-panel header strong { color: var(--text-secondary); font-size: 9.5px; font-weight: 550; }.effect-panel header span { flex: 1; }.effect-panel button { display: grid; width: 18px; height: 18px; place-items: center; padding: 0; color: var(--text-muted); background: transparent; border: 0; cursor: pointer; }.effect-parameter { display: grid; height: 25px; grid-template-columns: 1fr 68px 13px; align-items: center; gap: 4px; padding: 0 6px 0 24px; color: var(--text-muted); font-size: 9px; }.effect-parameter div { padding: 4px 5px; color: var(--text-secondary); text-align: right; background: var(--bg-input); border: 1px solid var(--border-strong); border-radius: 3px; }.effect-parameter svg { color: #5a5141; }.add-effect { display: flex; width: calc(100% - 12px); height: 27px; align-items: center; justify-content: center; gap: 5px; margin: 5px 6px 8px; color: var(--text-secondary); background: #181a20; border: 1px dashed #3a3e49; border-radius: 3px; font: inherit; font-size: 9.5px; cursor: pointer; }.add-effect:hover { color: var(--text-primary); border-color: var(--accent-border); }
