...
import { Box, Camera, Diamond, KeyRound, Magnet, Pause, Play, SkipBack, SkipForward, Sun, Trash2, ZoomIn, ZoomOut } from '@lucide/vue'
...
const entityIcon = computed(() => selectedSceneEntity.value?.kind === 'camera' ? Camera : selectedSceneEntity.value?.kind === 'light' ? Sun : Box)
...
<template>
  <section class="three-timeline">
    <header class="timeline-toolbar">
      ...
      <IconButton :icon="ZoomOut" label="Zoom timeline out" @click="timelineZoom = Math.max(50, timelineZoom - 20)" />
      <span class="zoom-value">{{ timelineZoom }}%</span>
      <IconButton :icon="ZoomIn" label="Zoom timeline in" @click="timelineZoom = Math.min(500, timelineZoom + 20)" />
    </header>

    <div ref="viewport" class="timeline-scroll">
      <div class="timeline-content" :style="contentStyle">
        ...
        <div v-else class="empty-timeline"><Box :size="18" /><strong>No 3D entity selected</strong><span>Select an object, camera, or light to animate its properties.</span></div>
