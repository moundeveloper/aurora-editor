...
<style scoped>
.physics-controls { display: flex; flex-direction: column; gap: 7px; padding: 9px; color: var(--text-secondary); border-bottom: 1px solid var(--border-subtle); font-size: 10px; }.physics-controls > strong { color: var(--text-primary); font-weight: 620; } label { display: flex; min-width: 0; align-items: center; justify-content: space-between; gap: 8px; } button { height: 24px; padding: 0 7px; color: inherit; background: var(--bg-input); border: 1px solid var(--border-strong); border-radius: 3px; font: inherit; cursor: pointer; white-space: nowrap; } button:hover:not(:disabled) { color: var(--text-primary); background: var(--bg-hover); border-color: var(--accent-border); } button:disabled { opacity: .42; cursor: default; } small, p { margin: 0; color: var(--text-muted); line-height: 1.4; } p[role=status] { padding: 5px 6px; color: var(--accent); background: var(--bg-selected); border: 1px solid var(--accent-border); border-radius: 3px; }
</style>
