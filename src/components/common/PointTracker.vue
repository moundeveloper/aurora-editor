...
<style scoped>
.point-tracker { display: grid; gap: 7px; padding: 9px; color: var(--text-secondary); border-bottom: 1px solid var(--border-subtle); font-size: 10px; } summary { margin-bottom: 1px; color: var(--text-muted); cursor: pointer; } label { display: flex; min-width: 0; align-items: center; justify-content: space-between; gap: 8px; } canvas { width: 100%; height: auto; background: #0b0d11; border: 1px solid var(--border-strong); border-radius: 3px; cursor: crosshair; } small { display: block; margin: 0; color: var(--text-muted); line-height: 1.4; } button { height: 24px; width: fit-content; padding: 0 7px; color: inherit; background: var(--bg-input); border: 1px solid var(--border-strong); border-radius: 3px; font: inherit; cursor: pointer; white-space: nowrap; } button:hover:not(:disabled) { color: var(--text-primary); background: var(--bg-hover); border-color: var(--accent-border); } button:disabled { opacity: .42; cursor: default; } p { margin: 0; color: var(--accent); line-height: 1.4; }
</style>
