...
const { selectedScene, selectedSceneEntity } = storeToRefs(store)
    ...
    <div v-if="selectedSceneEntity && transform" class="inspector-scroll">
        ...
        <div v-if="!collapsed.transform" class="transform-groups">
          <div v-for="group in (['position', 'rotation', 'scale'] as const)" :key="group" class="transform-group">
            ...
            <label v-for="axis in (['x', 'y', 'z'] as const)" :key="axis" :class="axis">
              ...
              <input :value="transform[group][axis].value" type="number" :step="group === 'rotation' ? 1 : .1" @input="setTransform(group, axis, $event)" />
              <small>{{ group === 'rotation' ? '°' : '' }}</small>
              <button type="button" :title="`Animate ${group} ${axis}`" :class="{ animated: transform[group][axis].animated }"><Diamond :size="9" :fill="transform[group][axis].animated ? 'currentColor' : 'none'" /></button>
            </label>
          </div>
        </div>
      </section>
      ...
      <section v-if="selectedSceneEntity.kind === 'object'" class="property-section">
        ...
        <div v-if="!collapsed.material" class="property-list">
          ...
          <label v-for="property in (['metalness', 'roughness', 'opacity', 'emissiveIntensity'] as const)" :key="property"><span>{{ property }}</span><input :value="selectedSceneEntity.value.material[property].value" type="number" min="0" :max="property === 'emissiveIntensity' ? 10 : 1" step=".05" @input="store.set3DObjectMaterial(property, Number(($event.target as HTMLInputElement).value))" /></label>
      ...
      <section v-if="selectedSceneEntity.kind === 'camera'" class="property-section">
        ...
        <div class="property-list"><label><span>Field of view</span><input v-model.number="selectedSceneEntity.value.fov.value" type="number" min="1" max="179" @input="store.markSceneChanged()" /></label><label><span>Near</span><input v-model.number="selectedSceneEntity.value.near" type="number" min=".001" step=".1" @input="store.markSceneChanged()" /></label><label><span>Far</span><input v-model.number="selectedSceneEntity.value.far" type="number" min="1" step="10" @input="store.markSceneChanged()" /></label><button class="active-camera" type="button" :disabled="selectedScene?.activeCameraId === selectedSceneEntity.value.id" @click="store.setActive3DCamera(selectedSceneEntity.value.id)">{{ selectedScene?.activeCameraId === selectedSceneEntity.value.id ? 'Active render camera' : 'Set as active camera' }}</button></div>
      </section>
      ...
      <section v-if="selectedSceneEntity.kind === 'light'" class="property-section">
        ...
        <div class="property-list"><label><span>Color</span><input class="color-field" v-model="selectedSceneEntity.value.color" type="color" @input="store.markSceneChanged()" /></label><label><span>Intensity</span><input :value="selectedSceneEntity.value.intensity.value" type="number" min="0" step=".1" @input="store.set3DLightIntensity(Number(($event.target as HTMLInputElement).value))" /></label><label class="check-row"><span>Cast shadows</span><button type="button" :class="{ checked: selectedSceneEntity.value.castShadow }" @click="selectedSceneEntity.value.castShadow = !selectedSceneEntity.value.castShadow; store.markSceneChanged()"><CircleDot :size="10" /></button></label></div>
      </section>
