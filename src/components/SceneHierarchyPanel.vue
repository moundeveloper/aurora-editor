...
function onDocumentPointerDown(event: PointerEvent) {
  ...
  if (!target?.closest('.add-menu, .add-menu-trigger')) addMenu.value = null
}

function onKeydown(event: KeyboardEvent) {
  if (event.key === 'Escape') addMenu.value = null
}
    ...
    <div class="hierarchy-scroll">
      <section>
        ...
        <template v-if="sections.cameras">
          <button v-for="camera in selectedScene?.cameras" :key="camera.id" class="entity-row" type="button" :class="{ active: selectedSceneEntityId === camera.id }" :title="camera.name" @click="select(camera.id)"><span class="indent" /><Camera :size="11" /><span>{{ camera.name }}</span><small v-if="programCameraId === camera.id">LIVE</small></button>
        </template>
      </section>
      <section>
