...
export class AuroraProjectDatabase extends Dexie {
  ...
  async saveSnapshot(snapshot: SerializedEditorState) {
    ...
    await this.transaction('rw', this.projects, this.layers, this.scenes3D, this.assets, this.settings, async () => {
  ...
  async loadSnapshot(projectId: string): Promise<SerializedEditorState | null> {
    return this.transaction('r', this.projects, this.layers, this.scenes3D, this.assets, async () => {
      const project = await this.projects.get(projectId)
      if (!project) return null
      const [layers, scenes3D, assets] = await Promise.all([
